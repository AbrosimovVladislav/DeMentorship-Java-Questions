...
- В чем идея тред пула? 
- Почему юзать пулы а не создавать поток по требованию? 
- Нужно ли закрывать поток?
- Как работать с синхронизацией потоков?
- Что такое интерфейс Future?
- Volatile что за спец слово
- Какие есть инструменты для работы с многопоточкой ? (syncronized, пулы, future/completableFuture, parallelStream, atomics)
- Есть общий ресурс, как обрабатывать его в многопоточке (syncronized, request scope в спринге)
